...
If you want to give the primary key constraint an explicit name in the database, use the ``name`` option. It is allowed for composite primary keys:

.. code-block:: python

    PrimaryKey(a, b, name="pk_example")

If you use ``auto=True`` for a single-column integer primary key on PostgreSQL, Pony generates the ``SERIAL`` type. You can generate the modern ``GENERATED BY DEFAULT AS IDENTITY`` instead by specifying ``auto="identity"`` (PostgreSQL only; other databases raise ``TypeError``):
...
In the database ``unique(a, b)`` will be represented as the ``UNIQUE ("a", "b")`` constraint. The older name ``composite_key()`` is kept as a deprecated alias of ``unique()`` and behaves the same way.

You can specify an explicit name for the constraint using the ``name`` option:

.. code-block:: python

    unique(a, b, name="unq_example_a_b")
...
You can also give a single-column unique constraint an explicit name by passing a string to ``unique``:

.. code-block:: python

    class Product(db.Entity):
        name = Required(str, unique="unq_product_name")
...
You can give an index an explicit name using the ``name`` option:
...
.. code-block:: python
    ...
    class Example(db.Entity):
        a = Required(str)
        b = Optional(int)
        composite_index(a, b, name="idx_example_a_b")

PostgreSQL supports additional index options. All of them raise ``TypeError`` on other databases:

* ``using`` — the index method (btree, hash, gin, gist or brin): ``composite_index(a, b, using="gin")``. Arrays are indexed with GIN automatically unless overridden.
* ``where`` — a partial index: ``composite_index(a, b, where="b > 0")``. The value is a raw SQL string. Works for attribute-level ``index`` and ``unique`` options too.
* ``desc()`` — the column order: ``composite_index(a, desc(b))``.
* ``include`` — covering columns: ``composite_index(a, b, include=(c,))``. The included attributes must not be part of the key.
* ``raw_sql()`` — an expression index: ``composite_index(raw_sql("lower(a)"), name="idx_lower")``. An explicit ``name`` is required, and only one expression is allowed per index.
* ``nulls_not_distinct`` — ``unique(a, b, nulls_not_distinct=True)`` (PostgreSQL 15+).
...
In the database this will be represented as ``CONSTRAINT "chk_person__adult" CHECK ("age" > 18)``. You can specify an explicit name with the ``name`` option:
...
The method remains a regular method of the entity and can be called as usual. Aggregates and subqueries are not allowed. Check constraints defined in base classes are merged into derived entities for single-table inheritance.
...
On PostgreSQL, Pony can add comments to tables and columns. The docstring of an entity becomes the table comment, and the ``comment`` attribute option becomes the column comment:
...
This generates ``COMMENT ON TABLE "person" IS 'People and their profiles.'`` and ``COMMENT ON COLUMN "person"."name" IS 'Full name'``. On other databases ``comment`` and docstrings are ignored.
